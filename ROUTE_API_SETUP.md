...
2. 註冊/登錄賬號
3. 創建應用
4. **添加 Key 時請選擇「Web 服務」類型**（本應用通過 HTTP 調用 restapi.amap.com，必須使用 Web 服務 Key，不能僅用 Android Key）
5. 為該 Key 添加服務：路線規劃、地理編碼
...
### API Key 平台不匹配錯誤（USERKEY_PLAT_NOMATCH）
如果遇到 `USERKEY_PLAT_NOMATCH` 錯誤，表示當前使用的 Key 不是 **Web 服務** 類型，而本應用是通過 HTTP 調用 `restapi.amap.com`，必須使用 **Web 服務** Key。解決方法：

1. **登錄 [高德開放平台控制台](https://console.amap.com/dev/key/app)**，進入您創建的應用。

2. **新增或修改 Key**：
   - 若當前只有 Android / iOS Key：請**新增一個 Key**，平台類型選擇 **「Web 服務」**。
   - 若已有 Key 但類型不對：無法直接改類型，需**新建一個「Web 服務」類型的 Key**。

3. **為該 Web 服務 Key 勾選服務**：
   - **地理編碼**
   - **路線規劃**（駕車路線規劃等）

4. **在代碼中替換 API Key**：
   - 在 `RoutePlanner.java` 中將 `AMAP_API_KEY` 改為上述 **Web 服務** Key 的值。
   - 重新編譯並運行應用。

完成後，語音識別 → 獲取位置 → 路線規劃 會正常播報結果或明確的錯誤提示。
